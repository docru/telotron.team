...
**Стадия:** MVP **в production** с 11 июня 2026. Сейчас идёт **пилот** (июнь–август): бесплатный триал, без приёма платежей. Цель пилота — доказать ценность на реальных тренерах, нарастить базу и подготовить коммерцию к **1 сентября 2026**.
...
**Потолок SaaS-выручки (грубо):** зависит от **числа клиенто-дней** и подключенных аддонов; прежняя рамка «~5 000 ₽/мес × N платящих» — **архив** (была при тарифной сетке). Новая калькуляция — после фиксации ставки клиенто-дня.
...
### Модель: B2B · платит тренер

Клиент платформе **не** платит.

**Канон · DEC-007 (2026-08-16):** тарифов-ступеней (**Лайт / Профи / Максимальный**) **нет**.

| Слой | Как платим |
|------|------------|
| **Ядро** (основной функционал) | **Плата за каждый день пользования клиентом** (клиенто-день). Ставка и определение — при реализации |
| **Аддоны** | Отдельная **абонплата** (пример: модуль «Поиск клиентов») |

Канон: [модель-монетизации-платформа](../../../_telotron.ru/docs/Бизнес-требования/00-канон-mvp/модель-монетизации-платформа.md).

### Вход в оплату

1. Пробный / триальный период — **параметры TBD**.
2. С **01.09.2026** (ориентир) — оплаты через **ЮKassa**; списание по правилам клиенто-дня (+ аддоны).
3. Пилот до старта оплат — без gating коммерции.

### Механика оплаты (направление)

- Пополнение через **ЮKassa** (карты РФ, рубли).
- Внутренний **счёт**; ежедневный контур списания стыкуется с **клиенто-днями**.
- Недостаток средств / ограничение доступа — **TBD**; данные сохраняются.
- Комиссия эквайринга — включена в цену для тренера (ориентир).

### Партнёрская программа (ключевой канал роста)

Тренер приглашает **другого тренера** по многоразовой ссылке.

**Вознаграждение** — процент от платежей приглашённых:
...
До порога **S** начисления могут идти **скидкой на счёт**; свыше S — вывод после договора. База относительно клиенто-дня и аддонов — уточнить.

**Почему это важно:** отдельного отдела продаж нет. Рост = **сарафан тренер → тренер** + личные связи основателя + соцсети.

### Что в пилоте бесплатно

- Триал **30 дней**, **полный** функционал MVP.
- **Нет** gating по тарифам, **нет** приёма платежей в prod.
- Партнёрские ссылки работают для **учёта регистраций**; выплаты — с **01.09**.
...
На **реальных тренерах в триале** проверить ценность «день тренера и клиента» и **подготовить коммерцию к 01.09** — без приёма платежей в prod.
...
| Чем продавать с 01.09? | Клиенто-день + аддоны (DEC-007) + billing/partner stage ready |
...
| Тема | Когда |
|------|-------|
| Платежи, MRR, ЮKassa в prod | Этап 2, **01.09** |
| Gating по тарифам | **01.09** |
...
timeline
    ...
    31.08 : Закрытие пилота · billing ready
    01.09 : Старт платежей · этап Коммерция
...
| **31.08.2026** | Закрытие пилота | База тренеров; billing+partner ready |
| **01.09.2026** | Старт платежей | ЮKassa, gating, партнёрка full |
...
**Запасной план**, если billing не готов к 01.09: не включать платежи; продлить триал активным (+14 дней); пересмотреть темп целей.
...
Из ~30 личных контактов отобрать **5–10** с активными соцсетями и интересом к доп. доходу. В июле — тест реферальных ссылок; выплаты с 01.09.
...
Если польза **не** доказана к 31.07 — продление пилота без оплат + доработка продукта; billing 01.09 **не** стартует автоматически.
...
| UI/UX дизайнер | Подключён точечно | UX/UI Pro·Client, VK-обложки, one-pager |
...
| Billing не готов к 01.09 | Запасной план: продление триала |
...
| **Коммуникация пилота** | Формулировки «без оплат до 01.09», без обещания цен и выплат партнёрки |
...
### Слой «рост» · аддоны (не тариф)

Инструменты привлечения клиентов — **отдельные платные модули** ([E-006 Поиск клиентов](../../backlog/эпики/E-006-поиск-клиентов.md)), не «Максимальный тариф».
...
1. **Сначала** — доказать ценность операционки (пилот) и **Commerce** (01.09).
...
## Приложение C · Монетизация (DEC-007)

| Слой | Модель |
|------|--------|
| **Ядро** | Плата за **клиенто-день** (день пользования клиентом) |
| **Аддоны** | Отдельная абонплата (напр. «Поиск клиентов») |

Тарифы Лайт / Профи / Максимальный — **сняты**. Ставка клиенто-дня и цены аддонов — при реализации.  
Канон: [модель-монетизации-платформа](../../../_telotron.ru/docs/Бизнес-требования/00-канон-mvp/модель-монетизации-платформа.md).
...
| Юрпакет v2.0 | Подписка + партнёрка → **01.09** |
| Медицинские claims | Запрещены |
| Оплата | ЮKassa (с 01.09), чеки через провайдера |
