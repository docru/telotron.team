...
ONBOARDING_DIR = INSTR / "онбординг-тренеров"
...
OUT = ARTIFACTS / "Онбординг — инструкция для тренеров.docx"
OUT_PDF = READY / "Онбординг — инструкция для тренеров.pdf"
...
def prepare_markdown() -> None:
    text = (ONBOARDING_DIR / "Онбординг — инструкция для тренеров.md").read_text(encoding="utf-8")
    cut = text.split("## Для команды (не отправлять тренеру)")[0]
    for src_name, dst_name in PAIRS:
        for prefix in ("../скрины/", "скрины/", "онбординг-тренеров/", "../онбординг-тренеров/"):
            cut = cut.replace(f"{prefix}{src_name}", f"media/{dst_name}")
